...
public:
    int solve(int i, int buy, vector<int> &prices, int fee, int n, vector<vector<int>> &dp) {
        ...
        int profit = 0;
        if (buy) {
            int take = -prices[i] + solve(i + 1, 0, prices, fee, n, dp);
            int notTake = solve(i + 1, 1, prices, fee, n, dp);
            profit = max(take, notTake);
        }
        else {
            int sell = prices[i] - fee + solve(i + 1, 1, prices, fee, n, dp);
            int notSell = solve(i + 1, 0, prices, fee, n, dp);
